 # StateStrike Phase 1 データ基盤実装計画 v1
  ...
  - この計画を repo 内の文書として残したい場合は、あなたがこの本文を保存するか、Plan Mode を抜けた後に「この内容をファイルに書いて」と依頼して
    ください。今のモードでは私は repo-tracked file を書きません。
  - 実装後に mainnet 実データで smoke test をしたい場合は、12 銘柄 allowlist を runtime config としてあなたが用意してください。コード側ではハー
    ドコードしません。
  - 将来 phase 2 で user streams や live path に進むときは、systemd EnvironmentFile の配置場所と権限をあなたが決めてください。phase 1 ではまだ
    不要です。
